...
export default function ConfiguratorViewer({
  ...
  const { zones: textZones, isLoading: isLoadingZones } = useTextZones(null);
  const { fonts, isLoading: isLoadingFonts } = useFonts();
  const { logos, isLoading: isLoadingLogos } = useLogos(null);
  const { selectedDesign, selectDesign } = useDesignSelection();
  const { colors, updateColor, replaceColors, resetColors } = useColorSelection();
  const { modelUrl, textureMaps, materialMaps, modelId, isLoading: isLoadingModel } = useAutoLoadModel(null, null, productId);
  ...
  const fontsForNames = useFilteredFonts('names');
  const fontsForNumbers = useFilteredFonts('numbers');
